...
# class ImageBase64:
#     @classmethod
#     def image2base64(cls, image_path):
#         try:
...
#     @classmethod
#     def base642image(cls, base64_str, image_path):
#         try:
...
class ImageBase64:
    @staticmethod
    def image_as_base64(path, max_short_side_length: int = -1):
        ...
        if os.path.splitext(path)[1].lower() in ('.jpg', '.jpeg'):
            ...
            bs_head='data:image/jpeg;base64,'
        else:
            image.save(buffered, format='PNG')
            bs_head='data:image/png;base64,'
        
        return bs_head + base64.b64encode(buffered.getvalue()).decode('utf-8')
    @staticmethod
    def base64_to_image(image_base64: Union[bytes, str]):
...
if __name__ == '__main__':
    image_dir=r"D:\An\CODE\datasets\pdf\质能等价-维基百科_自由的百科全书"
    
    image_paths=sorted([os.path.join(image_dir,img) for img in os.listdir(image_dir) if img.endswith(".jpg") or img.endswith(".png")])
    print(image_paths)
    
    video_path=r"D:\An\CODE\datasets\video\2025-9-22.mp4"
    
    fetch_video({"video":video_path})
